...
_POWER_PLATFORM_CLIENT_ATTEMPTED = False


def _pp_debug(message):
...
async def get_power_platform_client(tenant_id):
    ...
    """
    global _POWER_PLATFORM_CLIENT_CACHE, _POWER_PLATFORM_CLIENT_ATTEMPTED

    if _POWER_PLATFORM_CLIENT_ATTEMPTED:
        return _POWER_PLATFORM_CLIENT_CACHE
    ...
    if preloaded_data:
        ...
        _POWER_PLATFORM_CLIENT_ATTEMPTED = True
        _POWER_PLATFORM_CLIENT_CACHE = preloaded_data
    ...
    try:
        ...
        _pp_debug(f"[DEBUG] Credential obtained: {type(credential)}")
        ...
        try:
            _pp_debug(f"[DEBUG] Fetching Power Platform environments...")
            ...
            _pp_debug(f"[DEBUG] Environment response status: {env_response.status_code}")
        except Exception as e:
            _pp_debug(f"[DEBUG] Environment fetch failed: {e}")
            await client.aclose()
        ...
        _POWER_PLATFORM_CLIENT_ATTEMPTED = True
        _POWER_PLATFORM_CLIENT_CACHE = client
        return client
