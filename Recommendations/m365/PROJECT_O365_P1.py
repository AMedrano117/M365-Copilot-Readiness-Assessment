    ...
    if m365_insights:
        ...
        if teams_active_users >= 100:
            recommendations.append(new_recommendation(
                ...
                recommendation=f"Pilot Project Plan 1 AI scenarios with a bounded group drawn from the {teams_active_users} active Teams users. Baseline project-status preparation time and review accuracy, test an agent against approved Dataverse project data, and expand only if the customer's measured quality, cycle-time, and risk criteria are met.",
        ...
        if deployment.get('available'):
            ...
            if user_count > 500 and has_teams:
                deployment_rec = new_recommendation(
                    ...
                    recommendation="Test an AI project assistant with a named owner and approved Dataverse project data. Baseline the effort and error rate for status reporting, validate generated answers and proposed updates with human review, and make an expand or stop decision using the customer's observed results.",
                    link_text="AI Project Management",
                    link_url="https://learn.microsoft.com/microsoft-copilot-studio/",
                    priority="High",
                    status="Success"
