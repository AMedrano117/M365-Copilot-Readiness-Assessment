...
async def get_recommendation(sku_name, status="Success", client=None, purview_client=None):
    ...
    deployment_recs = []
    if status == "Success" and source_is_complete(purview_client, "insider_risk", getattr(purview_client, "insider_risk", None)):
        ...
        if insider_risk_data.get('available'):
            ...
            if total_policies > 0:
                deployment_rec = new_recommendation(
                    ...
                    finding_key="purview.insider_risk.policies",
