            ...
            if has_pp and env_count > 0:
                ...
                if user_count > 200:
                    deployment_rec = new_recommendation(
                        ...
                        recommendation=f"Pilot one Dataverse-integrated agent in {env_types}: 1) choose a named business transaction and data owner, 2) {'use dev/sandbox for schema and permission testing and production only after approval' if env_prod > 0 else 'establish a governed test boundary before live data operations'}, 3) measure actual sessions and API consumption from a bounded subset of the {user_count} potential users, 4) validate least-privilege table and row access, latency, and failure handling, and 5) define customer-owned quality, risk, capacity, and expand/stop thresholds. Do not estimate demand from a fixed percentage of users.",
