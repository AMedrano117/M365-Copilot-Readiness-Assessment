            ...
            if has_pp and env_count > 0:
                ...
                if has_copilot and has_knowledge:
                    deployment_rec = new_recommendation(
                        ...
                        recommendation=f"Pilot a conversational agent strategy using {env_types}: 1) Select one support process with an accountable owner, 2) Ground answers only in approved content from the relevant subset of the {site_count} SharePoint sites, 3) {'Use dev/sandbox for agent testing and production only after approval' if env_prod > 0 else 'Establish a governed non-production test environment'}, 4) Test invocation with an approved subset of the {copilot_count} Copilot-enabled users, 5) Implement handoff-to-human for unresolved cases, 6) define the customer baseline, answer-quality and containment measures, review date, and expand/stop threshold before deployment.",
                    ...
                    deployment_rec = new_recommendation(
                        ...
                        recommendation=f"Test one knowledge-driven agent in {env_types}: 1) choose an approved subset of the {site_count} SharePoint sites, 2) validate retrieval permissions and answer citations, 3) use a department with an identifiable question volume, 4) {'use dev/sandbox for testing and promote only after an approval decision' if env_prod > 0 else 'create a governed non-production pilot boundary'}, 5) measure answer quality, unresolved-question handoff, user feedback, and content gaps against a customer-defined baseline. Expand only if the agreed quality and risk thresholds are met.",
